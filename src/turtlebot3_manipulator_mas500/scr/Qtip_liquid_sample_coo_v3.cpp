...
// READ JOINT STATE VALUES AT LIQUID
...
RCLCPP_INFO(logger, "Transformed Pose Before:");
RCLCPP_INFO(logger, "Position: x0=%.3f, y0=%.3f, z0=%.3f",
            temp_home.position.x,
            temp_home.position.y,
            temp_home.position.z);
  

// Print the transformed pose
RCLCPP_INFO(logger, "Transformed Pose After:");
RCLCPP_INFO(logger, "Position: x1=%.3f, y1=%.3f, z1=%.3f",
            temp_homeG.position.x,
            temp_homeG.position.y,
            temp_homeG.position.z);

RCLCPP_INFO(logger, "Orientation (quaternion): x=%.3f, y=%.3f, z=%.3f, w=%.3f",
            temp_homeG.orientation.x,
            temp_homeG.orientation.y,
            temp_homeG.orientation.z,
            temp_homeG.orientation.w);

  
  
  move_group_interface.setPoseTarget(temp_homeG);
  move_group_interface.setPlanningTime(10.0);  // <-- Add this line
...
// HOME POSITION 
  ...
  // Set the target pose for the arm
  // geometry_msgs::msg::Pose homeG = transformer.transformToGlobal(home);
